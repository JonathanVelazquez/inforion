...
def test_catalog_create():
    runner = CliRunner()
    result = runner.invoke(create, args='--ionfile credentials/credentials.ionapi --name CSVSchema2 '
...
def test_catalog_delete():
    ...
    runner = CliRunner()
    result = runner.invoke(create, args='--ionfile credentials/credentials.ionapi --name {} '
                                        '--schema_type DSV --schema data/catalog_schema.json --properties '
...
def test_datalake_upload():
    runner = CliRunner()
    result = runner.invoke(upload, args='--ionfile credentials/credentials.ionapi --schema CSVSchema2 --logical_id '
...
def test_datalake_list():
    runner = CliRunner()
    result = runner.invoke(datalake_list, args="--ionfile credentials/credentials.ionapi --list_filter "
...
def test_datalake_get():
    runner = CliRunner()
    result = runner.invoke(datalake_get, args="--ionfile credentials/credentials.ionapi "
